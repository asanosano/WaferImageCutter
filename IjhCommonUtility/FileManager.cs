...
namespace IjhCommonUtility
{
    ...
    public class FileManager
    {
        ...
        public static void MoveDirectory(string sourceDirPath, string destDirPath)
        {
            ...
            DirectoryInfo[] dirs = dir.GetDirectories();
            // If the destination directory doesn't exist, create it.
            if (Directory.Exists(destDirPath)) DeleteDirectory(destDirPath);
            Directory.CreateDirectory(destDirPath);
        ...
        /// <summary>
        /// ファイルの名前変更したパス取得(ファイル名の一部を置き換え)
        /// </summary>
        /// <param name="srcFilePath"></param>
        /// <param name="replaceFrom">置き換え元の文字列</param>
        /// <param name="replaceTo">置き換え後の文字列</param>
        public static string GetRenamedPath_Replace(string srcFilePath, string replaceFrom, string replaceTo)
        {
            string fileName = Path.GetFileName(srcFilePath);
            string newName = fileName.Replace(replaceFrom, replaceTo);
            string dstFullPath = Path.Combine(Path.GetDirectoryName(srcFilePath), newName);
            return dstFullPath;
        }
        /// <summary>
